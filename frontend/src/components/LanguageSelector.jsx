  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        ...
        <span className="font-medium flex-1 text-left text-sm">{currentLanguage.name}</span>
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute left-0 mt-2 w-full bg-white rounded-lg shadow-xl border border-gray-200 z-20 max-h-80 overflow-y-auto">
            {LANGUAGES.map((lang) => (
              <button
                key={lang.code}
                onClick={() => handleSelect(lang.code)}
                className={`w-full flex items-center space-x-3 px-4 py-2.5 hover:bg-gray-50 transition-colors ${
                  lang.code === i18n.language ? 'bg-blue-50 border-l-4 border-blue-600' : ''
                }`}
              >
